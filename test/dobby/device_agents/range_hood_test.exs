defmodule Dobby.DeviceAgents.RangeHoodTest do
  @moduledoc """
  ...
  device_agent_contract(Dobby.DeviceAgents.RangeHood,
  ...
  test "power and speed are somebody's doing; discovery and availability are not" do
    assert RangeHood.intervention?(:power)
    assert RangeHood.intervention?(:speed_percent)
    refute RangeHood.intervention?(:supports_speed)
    refute RangeHood.intervention?(:available)
  end
  ...
  defp booted_state do
    device = %Dobby.Home.Device{
      id: "range_hood:test",
      name: "test fan",
      agent_module: RangeHood,
      bindings: %{fan: "fan.test"},
      settings: %{}
    }

    RangeHood.new(id: device.id, state: RangeHood.initial_state(device)).state
  end

  defp sync(state, entity_state, attributes) do
    params = %{entity_id: state.entity_id, state: entity_state, attributes: attributes}

    case RangeHood.SyncState.run(params, %{state: state}) do
