defmodule Dobby.DeviceAgents.AirPurifierTest do
  @moduledoc """
  ...
  device_agent_contract(Dobby.DeviceAgents.AirPurifier,
    bindings: %{fan: "fan.contract"},
    entity: [entity_id: "fan.contract"],
    discovery: :manual,
    arrivals: [
      {%{result: :accepted, action: :set_power, power: :on}, %{available: true, power: :on},
  ...
  test "power and speed are somebody's doing; discovery and availability are not" do
    assert AirPurifier.intervention?(:power)
    assert AirPurifier.intervention?(:speed_percent)
    refute AirPurifier.intervention?(:supports_speed)
    refute AirPurifier.intervention?(:available)
  end

  defp booted_state do
    device = %Dobby.Home.Device{
      id: "air_purifier:test",
      name: "test fan",
      agent_module: AirPurifier,
      bindings: %{fan: "fan.test"},
      settings: %{}
    }

    AirPurifier.new(id: device.id, state: AirPurifier.initial_state(device)).state
  end

  defp sync(state, entity_state, attributes) do
    params = %{entity_id: state.entity_id, state: entity_state, attributes: attributes}

    case AirPurifier.SyncState.run(params, %{state: state}) do
