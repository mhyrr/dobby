defmodule Dobby.DeviceAgents.Dryer do
  @moduledoc """
  A dryer reports its own laundry cycle, even in a stacked pair.
  ...
  use Jido.Agent,
    name: "dryer",
    description: "Reports the dryer cycle, door, and remote readiness",
    signal_routes: [{"ha.state_changed", Dobby.DeviceAgents.Dryer.SyncState}],
    schema: [
  ...
  @impl Dobby.DeviceAgent
  def tools, do: [Dobby.Tools.DryerGetStatus]
  ...
  @impl Dobby.DeviceAgent
  def snapshot(state), do: ApplianceReadings.snapshot(state, :dryer)

  @doc false
  def sync(params, state), do: ApplianceReadings.sync(params, state, @reading_types, :dryer)
end
